  ...
  const handleExport = () => {
    ...
    if (documentData.fileUrl) {
      const fileUrl = documentData.fileUrl.startsWith("http")
        ...
        : `${process.env.REACT_APP_API_URL || "http://localhost:5001"}${documentData.fileUrl}`
                  ...
                  {documentData.fileType === "pdf" && documentData.fileUrl && (
                    <Tab.Panel className="px-8 py-6">
                      <div className="w-full h-[600px] bg-gray-100 rounded-lg overflow-hidden">
                        <iframe
                          src={`${process.env.REACT_APP_API_URL || "http://localhost:5001"}/api/documents/file/${documentData.fileUrl.split("/").pop()}`}
